...
import com.descope.model.user.request.*;
...
class UserServiceImpl extends ManagementsBase implements UserService {
    ...
    @Override
    public JsonNode update(String loginId, UserRequest request) throws DescopeException {
        ...
        var apiProxy = getApiProxy();
        return apiProxy.post(updateUserUri, request, JsonNode.class);
    }
    ...
    @Override
    public JsonNode load(String loginId) throws DescopeException {
        if (StringUtils.isBlank(loginId)) {
            throw ServerCommonException.invalidArgument("Login ID");
        }
        URI loadUserUri = composeLoadUserUri(Map.of("loginId", loginId));
        var apiProxy = getApiProxy();
        return apiProxy.get(loadUserUri, JsonNode.class);
    }
